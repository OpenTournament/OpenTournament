...
TSharedRef<FGitSourceControlState, ESPMode::ThreadSafe> FGitSourceControlProvider::GetStateInternal(const FString& Filename)
{
	...
	else
	{
		...
		TSharedRef<FGitSourceControlState, ESPMode::ThreadSafe> NewState = MakeShareable( new FGitSourceControlState(Filename, static_cast<bool>(bUsingGitLfsLocking)) );
...
{
	return static_cast<bool>(bUsingGitLfsLocking); // Git LFS Lock uses read-only state
}
