...
class SDockTabUMods : public SDockTab
{

private:

	FString Mod;
	TArray<FString> ModsNames;
	TArray<TSharedRef<SCheckBox>> ModsEntries;

	FString Release;
	TArray<FString> ReleasesNames;
	TArray<TSharedRef<SCheckBox>> ReleasesEntries;

public:

	void Set();

	void OnReleaseSelected(ECheckBoxState State);
