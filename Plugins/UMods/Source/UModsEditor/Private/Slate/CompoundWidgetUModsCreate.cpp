...
void SCompoundWidgetUModsCreate::Construct(const FArguments& InArgs)
{
	TSharedRef<SBorder> Outline = SNew(SBorder);
	{
		TSharedRef<SVerticalBox> VerticalBox = SNew(SVerticalBox);
		{
			...
			SVerticalBox::FSlot& VerticalBoxSlotB = VerticalBox->AddSlot();
			{
				...
				{
					SScrollBox::FSlot& ScrollBoxSlotA = ScrollBox->AddSlot();
					{
						TSharedRef<SCheckBoxUMods> CheckBoxUMods = SNew(SCheckBoxUMods);
						CheckBoxUMods->TextBlock->SetText(INVTEXT("Has Code"));
						CheckBoxUMods->OnSelected.BindRaw(this, &SCompoundWidgetUModsCreate::OnSelected);
						ScrollBoxSlotA.AttachWidget(CheckBoxUMods);
					}
					SScrollBox::FSlot& ScrollBoxSlotB = ScrollBox->AddSlot();
					{
						TSharedRef<SCheckBoxUMods> CheckBoxUMods = SNew(SCheckBoxUMods);
						CheckBoxUMods->TextBlock->SetText(INVTEXT("Has Code"));
						CheckBoxUMods->OnSelected.BindRaw(this, &SCompoundWidgetUModsCreate::OnSelected);
						ScrollBoxSlotB.AttachWidget(CheckBoxUMods);
					}
					// Configuration options for creating a mod.
				}
			...
			SVerticalBox::FSlot& VerticalBoxSlotC = VerticalBox->AddSlot();
			{
				TSharedRef<SBox> Box = SNew(SBox);
				{
					TSharedRef<SBorder> Border = SNew(SBorder);
					{
						...
						Button->TextBlock->SetText(FText::FromString(TEXT("Create")));
						Button->OnSelected.BindRaw(this, &SCompoundWidgetUModsCreate::OnButtonReleased);;
...
void SCompoundWidgetUModsCreate::OnSelected(FString Text)
{
	UE_LOG(LogTemp, Warning, TEXT("Selected: %s"), *Text);
}

void SCompoundWidgetUModsCreate::OnButtonReleased()
{
