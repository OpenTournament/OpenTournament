...
#include "UR_CameraMode_ThirdPerson.h"
...
#include "UR_CameraAssistInterface.h"
#include "Camera/UR_CameraMode.h"
#include "Camera/UR_PenetrationAvoidanceFeeler.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(UR_CameraMode_ThirdPerson)

/////////////////////////////////////////////////////////////////////////////////////////////////

namespace UR_CameraMode_ThirdPerson_Statics
{
...
UUR_CameraMode_ThirdPerson::UUR_CameraMode_ThirdPerson()
{
    TargetOffsetCurve = nullptr;

    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
    PenetrationAvoidanceFeelers.Add(FUR_PenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UUR_CameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UUR_CameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UUR_CameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
    ...
    for (int i = 0; i < DebugActorsHitDuringCameraPenetration.Num(); i++)
    {
        ...
        (
            FString::Printf(TEXT("HitActorDuringPenetration[%d]: %s"), i, *DebugActorsHitDuringCameraPenetration[i]->GetName()));
    }
...
void UUR_CameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
    ...
    IUR_CameraAssistInterface* TargetControllerAssist = Cast<IUR_CameraAssistInterface>(TargetController);

    IUR_CameraAssistInterface* TargetActorAssist = Cast<IUR_CameraAssistInterface>(TargetActor);
    ...
    IUR_CameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IUR_CameraAssistInterface>(PPActor) : nullptr;
    ...
    if (PPActorRootComponent)
    {
        ...
        IUR_CameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

        if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
        {
            for (IUR_CameraAssistInterface* Assist : AssistArray)
            {
                if (Assist)
                {
...
void UUR_CameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
    ...
    //TODO IUR_CameraTarget.GetIgnoredActorsForCameraPentration();
    ...
    {
        FUR_PenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
        if (Feeler.FramesUntilNextTrace <= 0)
        {
            ...
            if (bHit && HitActor)
            {
                ...
                if (HitActor->ActorHasTag(UR_CameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
                {
                    bIgnoreHit = true;
...
void UUR_CameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UUR_CameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
