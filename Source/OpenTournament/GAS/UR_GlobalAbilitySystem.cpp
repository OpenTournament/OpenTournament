...
#include UE_INLINE_GENERATED_CPP_BY_NAME(UR_GlobalAbilitySystem)

/////////////////////////////////////////////////////////////////////////////////////////////////

void FGlobalAppliedAbilityList::AddToASC(TSubclassOf<UGameplayAbility> Ability, UUR_AbilitySystemComponent* ASC)
{
...
void FGlobalAppliedAbilityList::RemoveFromASC(UUR_AbilitySystemComponent* ASC)
{
	if (FGameplayAbilitySpecHandle* SpecHandle = Handles.Find(ASC))
	{
...
void FGlobalAppliedEffectList::AddToASC(TSubclassOf<UGameplayEffect> Effect, UUR_AbilitySystemComponent* ASC)
{
...
void FGlobalAppliedEffectList::RemoveFromASC(UUR_AbilitySystemComponent* ASC)
{
	if (FActiveGameplayEffectHandle* EffectHandle = Handles.Find(ASC))
	{
...
UUR_GlobalAbilitySystem::UUR_GlobalAbilitySystem()
{
}

void UUR_GlobalAbilitySystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		FGlobalAppliedAbilityList& Entry = AppliedAbilities.Add(Ability);
		for (UUR_AbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void UUR_GlobalAbilitySystem::ApplyEffectToAll(TSubclassOf<UGameplayEffect> Effect)
{
...
void UUR_GlobalAbilitySystem::RemoveAbilityFromAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && AppliedAbilities.Contains(Ability))
	{
...
void UUR_GlobalAbilitySystem::RemoveEffectFromAll(TSubclassOf<UGameplayEffect> Effect)
{
	if ((Effect.Get() != nullptr) && AppliedEffects.Contains(Effect))
	{
...
void UUR_GlobalAbilitySystem::RegisterASC(UUR_AbilitySystemComponent* ASC)
{
	check(ASC);
...
void UUR_GlobalAbilitySystem::UnregisterASC(UUR_AbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
