...
#include "UR_PawnComponent_CharacterParts.generated.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FUR_SpawnedCharacterPartsChanged, UUR_PawnComponent_CharacterParts*, ComponentWithChangedParts);

/////////////////////////////////////////////////////////////////////////////////////////////////

// A single applied character part
USTRUCT()
struct FUR_AppliedCharacterPartEntry : public FFastArraySerializerItem
{
    GENERATED_BODY()

    FUR_AppliedCharacterPartEntry()
    {}
...
private:
    ...
    friend UUR_PawnComponent_CharacterParts;
...
struct FUR_CharacterPartList : public FFastArraySerializer
{
    GENERATED_BODY()

    FUR_CharacterPartList()
...
public:
    ...
    {
        return FFastArraySerializer::FastArrayDeltaSerialize<FUR_AppliedCharacterPartEntry, FUR_CharacterPartList>(Entries, DeltaParms, *this);
    }

    FUR_CharacterPartHandle AddEntry(FUR_CharacterPart NewPart);
    void RemoveEntry(FUR_CharacterPartHandle Handle);
    void ClearAllEntries(bool bBroadcastChangeDelegate);

    FGameplayTagContainer CollectCombinedTags() const;

    void SetOwnerComponent(UUR_PawnComponent_CharacterParts* InOwnerComponent)
    {
...
private:
    ...
    bool SpawnActorForEntry(FUR_AppliedCharacterPartEntry& Entry);
    bool DestroyActorForEntry(FUR_AppliedCharacterPartEntry& Entry);

private:
    // Replicated list of equipment entries
    UPROPERTY()
    TArray<FUR_AppliedCharacterPartEntry> Entries;

    // The component that contains this list
    UPROPERTY(NotReplicated)
    TObjectPtr<UUR_PawnComponent_CharacterParts> OwnerComponent;
...
template <>
struct TStructOpsTypeTraits<FUR_CharacterPartList> : public TStructOpsTypeTraitsBase2<FUR_CharacterPartList>
{
...
class UUR_PawnComponent_CharacterParts : public UPawnComponent
{
    GENERATED_BODY()

public:
    UUR_PawnComponent_CharacterParts(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
    ...
    FUR_CharacterPartHandle AddCharacterPart(const FUR_CharacterPart& NewPart);
    ...
    UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category=Cosmetics)
    void RemoveCharacterPart(FUR_CharacterPartHandle Handle);
