...
switch ($COMMAND)
{
    ...
    "Archive"
    {
        Write-Host | Compress-Archive -Path "$PROJECT_ROOT\Packages\$PROJECT_TITLE-$PACKAGE_TARGET-$PACKAGE_CONFIGURATION-$PACKAGE_PLATFORM\*" -DestinationPath "$PROJECT_ROOT\Packages\$PROJECT_TITLE-$PACKAGE_TARGET-$PACKAGE_CONFIGURATION-$PACKAGE_PLATFORM.zip"
        if ($LASTEXITCODE -ne 0)
        {
            exit $LASTEXITCODE;
        }
